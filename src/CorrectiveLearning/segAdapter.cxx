...
int main( int argc, char ** argv )
{
  if ( argc < 4 )
    {
    std::cerr << "segAdapter: Apply corrections to segmentations produced by the host segmentation method. " << std::endl;
    ...
    std::cerr << argv[0]
              ...
              << "                        Then feature0000.nii will be used for label 0 and feature0001.nii for label 1, etc." << endl
              << "  -m mask:              Specify ROI for the training images. Should be in C printf format, e.g. mask%04d.nii" <<endl
...
//    IteratorType imit0(im->GetOutput(), im->GetOutput()->GetRequestedRegion());
	...
	// load AdaBoost learning parameters 
        ...
        for (exclusionMaskit.GoToBegin(),segmaskit.GoToBegin(),csegit.GoToBegin(),nsegit.GoToBegin(),posteriorit.GoToBegin();!segmaskit.IsAtEnd(); ++segmaskit,++csegit,++nsegit,++exclusionMaskit,++posteriorit)
        {
          ImageType::IndexType idx = segmaskit.GetIndex();
          //excluding voxels which are too close to the edge of the image from consideration
          if (idx[0]<DX || idx[0]>=r-DX || idx[1]<DY || idx[1]>=c-DY || idx[2]<DZ || idx[2]>=d-DZ)
             continue;
          if (exclusionMaskit.Value()==0 && mask->GetPixel(idx)>0 && segmask->GetPixel(idx)>0 && cseg->GetPixel(idx)<1)
          {
