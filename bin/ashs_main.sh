  ...
  cat <<-USAGETEXT
		...
		optional:
		  ...
		  -r files          Compare segmentation results with a reference segmentation. The parameter
		                    ...
		                    and the ASHS segs.
		  -m file <0|1>     Provide the .mat file for the transform between the T1w and T2w image. The file
		                    ...
		                    The second parameter specifies whether the matrix file should be used to
		                      0: initialize T1-T2 registration
		                      1: specify the T1-T2 alignment (no registration by ASHS)
		                    This option is useful if ASHS fails due to poor T1/T2 registration.
...
unset ASHS_INPUT_T2T1_MATRIX_OPT
...
# Read the options
while getopts "g:f:w:s:a:q:I:C:r:z:m:HNTdhVQP" opt; do
  case $opt in
    ...
    m) ASHS_INPUT_MATRIX_OPTS=($(echo $OPTARG));;
    d) set -x -e;;
    h) usage; exit 0;;
...
if [[ $ASHS_INPUT_MATRIX_OPTS ]]; then

  # Split the options into a file and mode
  read ASHS_INPUT_T2T1_MAT ASHS_INPUT_T2T1_MODE <<< ${ASHS_INPUT_MATRIX_OPTS}
  ...
  if [[ ! -f $ASHS_INPUT_T2T1_MAT ]]; then
    ...
    echo "User-supplied T2->T1 transform: ${ASHS_INPUT_T2T1_MAT}"
  fi

  # Check that the mode is correct
  if [[ $ASHS_INPUT_T2T1_MODE -ne 0 && $ASHS_INPUT_T2T1_MODE -ne 1 ]]; then
    echo "Second parameter to -m must be 0 or 1"
    exit -2
  fi
fi
